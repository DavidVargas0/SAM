...
namespace  SAM.Analytical
{
    ...
    {
        public static List<Zone> MapZones(this AdjacencyCluster adjacencyCluster, TextMap textMap)
        {
            ...
            foreach (string zoneCategory in zoneCategories)
            {
                ...
                foreach (string parameterName in parameterNames)
                {
                    ...
                    {
                        Zone zone = adjacencyCluster.UpdateZone(keyValuePair.Key, zoneCategory, keyValuePair.Value?.ToArray());
                        if (zone == null)
                            continue;
